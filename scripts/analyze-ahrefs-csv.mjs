  ...
  meta: {
    ...
    users: {
      ...
      esteban: {
        ...
        role: 'Propietario',
        channel: 'Orbit proxy · Supabase Edge · monitores',
      },
    },
  },
  attack: {
    title: 'El CSV identifica al quemador',
    bullets: [
      `Ciclo anterior: Accesos SeoLab (MCP) gastó ${u(previous, 'Accesos').toLocaleString('es-CO')} units (${previous.mcp_pct}% del mes) — OpenAI MCP lideró.`,
      `Ciclo actual (${current.days} días): Orbit/Esteban ${u(current, 'Online').toLocaleString('es-CO')} vs MCP/Accesos ${u(current, 'Accesos').toLocaleString('es-CO')}.`,
      'Llamadas MCP matching-terms de hasta 13.250 units c/u (13 jul: dos = 26.500).',
      'MCP no deja IP. Orbit deja rastro AWS US (Supabase Edge).',
      'Endpoints caros: keywords-explorer/* y site-explorer/organic-keywords — no el panel UI.',
    ],
  },
  compare: {
    previous: {
